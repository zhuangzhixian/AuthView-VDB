"""Phase 2B-3b: partial-visible policy-integrated AuthView ZK proof tests."""
...
def _run_policy_proof(idx, inputs, user, checkpoint, labels):
    ...
    return metrics, ordered, candidates


def test_auth_zk_partial_visible_matches_plaintext_oracle(synthetic_zk_index):
    ...
    metrics, ordered, candidates = _run_policy_proof(
    ...
    for sc in result.scored:
        if sc.cid in invisible_cids and sc.valid:
            ...
            assert sc.masked_distance == (1 << 62) - 1
    for cid in oracle_topk:
        assert cid not in invisible_cids

    _build, _prove, verify_time, _size, _mem, _gates = metrics
    assert verify_time > 0.0


def test_auth_zk_policy_all_visible_matches_all_visible_path(synthetic_zk_index):
    idx = synthetic_zk_index
    inputs = _build_merkle_proof_inputs(
        idx["query"],
        idx["center"],
    ...
    policy_metrics, ordered, candidates = _run_policy_proof(
        idx, inputs, user, checkpoint, labels
    )
    ...
    policy_topk = top_k_cids_from_ordered(ordered, idx["top_k"])
    assert compare_with_v3db_topk(policy_topk, oracle_topk)
    ...
    all_visible_metrics = py_set_based_auth_all_visible_with_merkle(*common_v3db_args)

    _b1, _p1, verify_policy, _, _, gates_policy = policy_metrics
    _b2, _p2, verify_allvis, _, _, gates_allvis = all_visible_metrics
    assert verify_policy > 0.0
    assert verify_allvis > 0.0
    assert gates_policy >= gates_allvis
